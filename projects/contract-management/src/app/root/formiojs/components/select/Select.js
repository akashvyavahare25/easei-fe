...
var SelectComponent = /*#__PURE__*/function (_Field) {
  ...
  _createClass(SelectComponent, [{
    ...
    value: function loadItems(url, search, headers, options, method, body) {
      ...
      var query = this.component.dataSrc === 'url' || this.component.dataSrc === 'master' || this.component.dataSrc === 'screen' ? {} : {
     //   limit: limit,
        //skip: skip
      }; // Allow for url interpolation.
      url = this.interpolate(url, {
        formioBase:  env.environment.baseUrl,
        search: search,
        limit: limit,
        skip: skip,
      ...
      this.loading = true;
      var projectBaseUrl =  localStorage.getItem('baseUrl');
      if (this.component.key === 'data.master') {
        headers.set('Authorization', 'Bearer ' + localStorage.getItem('jwtToken'));
        options.header = headers;
        _Formio.default.makeRequest(this.options.formio, 'select', env.environment.baseUrl + '/api/master', method, body, options).then(function (res) {
        const newResponse = []
          res.forEach((r) => {
            newResponse.push({ data: r })
          });
          _this3.setItems(newResponse, !!search);
        });
      } else if (this.component.key === 'data.screen') {
        headers.set('Authorization', 'Bearer ' + localStorage.getItem('jwtToken'));
        options.header = headers;
        _Formio.default.makeRequest(this.options.formio, 'select', env.environment.baseUrl + '/api/screen', method, body, options).then(function (res) {
        const newResponse = []
          res.forEach((r) => {
            newResponse.push({ data: r })
          });
        ...
        _Formio.default.makeRequest(this.options.formio, 'select', url, method, body, options).then(function (response) {
         ...
         if(response.datasource){
          _this3.setItems(response.datasource, !!search);
         }else{
  ...
  },
  {
    ...
    value: function updateItems(searchInput, forceUpdate) {
      ...
      var _this = this;
      var projectBaseUrl = localStorage.getItem('baseUrl');
      if (this.component.dataSrc === 'master') {
        if (this.component.data.master !== '') {
          var options = {};
          var list = [];
          var headers = this.requestHeaders
          headers.set('Authorization', 'Bearer ' + localStorage.getItem('jwtToken'));
          options.header = headers;
          var name = _this.component.data.master.code;
          var namvar = this.interpolate(_this.component.quryValmaster);
          var extraNamvar = '';
          if (namvar && namvar.length > 0) {
            // const pp = _.split(namvar, '$');
            // for (var i = 0; i < pp.length; i++) {
            //   if (_.includes(pp[i], '#')) {
            //     var mm = _.split(pp[i], '#');
            //     extraNamvar = extraNamvar + mm[0] + _this.dataNew[mm[1]]
            //   } else {
            //     extraNamvar = extraNamvar + pp[i];
            //   }
            // }
            //console.log("hiiiiiiiiiiiiiiiiiiiiiiiiiiiiii",env.environment.baseUrl)
            _Formio.default.makeRequest(this.options.formio, 'select', env.environment.baseUrl + '/api/query/master/' + name + '?' + namvar, method, body, options).then(function (res) { 
            _this.setItems(res.datasource);
            });
          } else {
            let jsonObj = "%7B%7D"
            _Formio.default.makeRequest(this.options.formio, 'select', env.environment.baseUrl + '/api/eimaster/name/false/'+ name+`?json=%7B%7D`, method, body, options).then(function (res) {
            _this.setItems(res);
            });
          }
        }
      } else if (this.component.dataSrc === 'screen') {
        if (this.component.data.screen !== '') {
          var options = {};
          var list = [];
          var headers = this.requestHeaders
          headers.set('Authorization', 'Bearer ' + localStorage.getItem('jwtToken'));
          options.header = headers;
          var name = _this.component.data.screen.code;
          var namvarS = _this.component.quryValscreen;
          if (namvarS.length > 0) {
            _Formio.default.makeRequest(this.options.formio, 'select', env.environment.baseUrl + '/api/query/screen/' + name + '?' + namvarS, method, body, options).then(function (res) { 
            /* res.datasource.forEach((r) => {
                var object = {
                  value: r._id,
                  label: r[res.metadata.uniqueKey[0]]
                }
            ...
            _Formio.default.makeRequest(this.options.formio, 'select', env.environment.baseUrl + '/api/lookup/screen/' + name, method, body, options).then(function (res) {
            /* res.datasource.forEach((r) => {
                var object = {
                  value: r._id,
                  label: r[res.metadata.uniqueKey[0]]
          ...
          case 'resource':
            {
              ...
              var resourceUrl = this.options.formio ? this.options.formio.formsUrl : "".concat(_Formio.default.getProjectUrl(), "/api/eimaster");
              resourceUrl += "/".concat(this.component.data.resource, "");
            
              if (forceUpdate || this.additionalResourcesAvailable || this.dataValue.length && !this.serverCount || this.interpolate(this.component.filter)) {
                try {
    ...
    get: function get() {
      ...
      if (this.component.dataSrc === 'screen') {
        return '';
      }
    ...
    key: "isSelectURL",
    get: function get() {
      return this.component.dataSrc === 'master' || this.component.dataSrc === 'screen';
    }
  },
  {
